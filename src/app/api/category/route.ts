...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, title, slug, summary, featured_image, category, author, published_at, views FROM articles WHERE site_id = $1 AND category IN (${placeholders}) ORDER BY published_at DESC LIMIT ${perPage} OFFSET ${offset}`,
      [siteId, ...variants]
    );
